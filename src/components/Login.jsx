...
const Login = () => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} onClick={(e)=> e.stopPropagation()} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 mx-5 bg-white">
                ...
                {state !== "login" && (
                    <div className="flex items-center mt-6 w-full bg-white border border-purple-600 h-12 rounded-full overflow-hidden pl-6 gap-2">
                ...
                <div className="flex items-center w-full mt-4 bg-white border border-purple-600 h-12 rounded-full overflow-hidden pl-6 gap-2">
                ...
                <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="text-gray-500 text-sm mt-3 mb-11">{state === "login" ? "Don't have an account?" : "Already have an account?"} <a href="#" className="text-purple-600 hover:underline">click here</a></p>
            </form>
    </div>
  )
}
