...
const CartDetails = () => {
    const { products, getCartCount, getCartAmount, navigate, currency, cartItem, deleteCartItem, updateCartItem} = useAppContext();

    const [cartArray, setCartArray] = useState([])
    const [addresses, setAddresses] = useState(dummyAddress)
    const [showAddress, setShowAddress] = useState(false);
    const [selectedAddress, setSelectedAddresss] = useState(dummyAddress[0]);
            ...
            <div className='flex-1 max-w-4xl px-5'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={()=> deleteCartItem(product._id)} className="cursor-pointer mx-auto">
                            <img src={assets.remove_icon} className='inline-block w-6 h-6' />
                        </button>
                    </div>)
                )}

                <button onClick={()=> {navigate(`/products`); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-purple-700 font-medium" >
                    <FaArrowLeftLong className='group-hover:-translate-x-1 transition text-2xl'/>
                    Continue Shopping
                </button>

            </div>

            <div className="max-w-[390px] w-full bg-gray-100/40 p-5 max-md:mt-16 mr-10 border border-gray-300/70">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                ...
                                <p onClick={() => {navigate("/add-address"); scrollTo(0,0)}} className="text-purple-500 text-center cursor-pointer p-2 hover:bg-purple-500/10">
                                    Add address
                                </p>
                            </div>
                        )}
